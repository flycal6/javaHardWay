...
public class StudentDatabase {
    public static void main(String[] args) {
        ...
        int fewCreds = 0;
        for (int i=1; i<db.length; i++)
            if (db[i].credits < db[fewCreds].credits)
                fewCreds = i;

        System.out.println(db[fewCreds].name + " has the fewest credits.");
    }
}
// SD #1 The program blows up because the field is null.
